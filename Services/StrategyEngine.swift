...
/// 增强策略引擎类 - 集成完整技术指标库
@MainActor
class StrategyEngine: ObservableObject {
    ...
    func enableStrategy(_ strategyId: UUID) async {
        ...
        // print("策略已启用: \(strategies[index].name)") // 调试语句已注释
    }
    ...
    func disableStrategy(_ strategyId: UUID) async {
        ...
        // print("策略已禁用: \(strategies[index].name)") // 调试语句已注释
    }
    ...
    private func saveStrategyToStorage(_ strategy: AdvancedStrategy) async {
        ...
        // print("保存策略: \(strategy.name)") // 调试语句已注释
    }
    ...
    private func executeSignal(_ signal: EnhancedTradingSignal) async {
        ...
        // print("执行交易信号: \(signal.symbol) \(signal.signalType.rawValue) 置信度: \(signal.confidence)") // 调试语句已注释
        // print("理由: \(signal.reasoning)") // 调试语句已注释
    }
...
typealias StrategyEngine = EnhancedStrategyEngine