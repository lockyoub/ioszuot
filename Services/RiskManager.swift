...
public class RiskManager: ObservableObject {
    ...
    private func getInitialAccountValue() -> Double {
        // 从持久化存储中获取初始账户价值，如果没有则使用配置的默认值
        let userDefaults = UserDefaults.standard
        let storedValue = userDefaults.double(forKey: "initial_account_value")
        
        if storedValue > 0 {
            return storedValue
        } else {
            // 使用配置管理器的默认值
            let defaultValue = ConfigurationManager.shared.maxPositionSize
            userDefaults.set(defaultValue, forKey: "initial_account_value")
            return defaultValue
        }
    }
    
    /// 获取现金余额
    private func getCashBalance() -> Double {
        // 从持久化存储中获取当前现金余额
        let userDefaults = UserDefaults.standard
        let storedBalance = userDefaults.double(forKey: "cash_balance")
        
        if storedBalance > 0 {
            return storedBalance
        } else {
            // 使用配置管理器的默认值
            let defaultBalance = getInitialAccountValue()
            userDefaults.set(defaultBalance, forKey: "cash_balance")
            return defaultBalance
        }
    }
    
    /// 更新现金余额（在交易时调用）
    public func updateCashBalance(_ newBalance: Double) {
        UserDefaults.standard.set(newBalance, forKey: "cash_balance")
    }
...
// MARK: - 测试兼容方法
extension RiskManager {
    /// 为测试提供的简化风险检查方法
    /// - Parameter order: 订单请求
    /// - Returns: 风险检查结果
    public func check(order: OrderRequest) -> RiskCheckResult {
        // 检查订单数量是否超过限制
        if order.quantity > 10000 {
            return .rejected(reason: "数量超过限制")
        }
        
        // 检查价格是否合理
        if order.price <= 0 {
            return .rejected(reason: "价格无效")
        }
        
        // 检查持仓风险
        let positionValue = Double(order.quantity) * order.price
        let riskResult = checkPositionRisk(symbol: order.symbol, quantity: order.quantity, price: order.price)
