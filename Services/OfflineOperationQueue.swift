...
class OfflineOperationQueue: ObservableObject {
    ...
    private let networkService = EnhancedNetworkManager.shared
    private var isProcessing = false
    
    // MARK: - Logger
    private let logger = Logger(subsystem: "com.stocktrading.app", category: "OfflineQueue")
    ...
    private func processQueueItem(_ item: OfflineQueueEntity, context: NSManagedObjectContext) async {
        ...
        do {
            ...
            case .createTrade:
                try await networkService.post("/api/trades", body: operationData)
            case .updateTrade:
                try await networkService.put("/api/trades", body: operationData)
            case .deleteTrade:
                try await networkService.delete("/api/trades/\(operationData[\"id\"] as? String ?? \"\")")
            case .updatePosition:
                try await networkService.put("/api/positions", body: operationData)
            }
