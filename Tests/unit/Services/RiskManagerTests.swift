...
final class RiskManagerTests: BaseTestCase {
    ...
    func testInitialization() throws {
        // 测试初始化
        // XCTAssertNotNil(riskmanager, "RiskManager应该成功初始化")}
    
    // MARK: - 核心功能测试
    
    func testManagerConfiguration() throws {
        // 测试管理器配置
        let config = RiskManagerConfiguration()
        riskmanager.configure(with: config)
        // XCTAssertTrue(riskmanager.isConfigured, "管理器应该配置成功")}
