...
final class AuthManagerTests: BaseTestCase {
    ...
    func testInitialization() throws {
        // 测试初始化
        // XCTAssertNotNil(authmanager, "AuthManager应该成功初始化")}
    
    // MARK: - 核心功能测试
    
    func testManagerConfiguration() throws {
        // 测试管理器配置
        let config = AuthManagerConfiguration()
        authmanager.configure(with: config)
        // XCTAssertTrue(authmanager.isConfigured, "管理器应该配置成功")}
