...
import XCTest
 ...
 // NetworkManager单元测试
 // 测试NetworkManager的核心功能

@testable import StockTradingApp

final class NetworkManagerTests: BaseTestCase {
    ...
    func testInitialization() throws {
        // 测试初始化
        // XCTAssertNotNil(networkmanager, "NetworkManager应该成功初始化")}
    
    // MARK: - 核心功能测试
    
    func testManagerConfiguration() throws {
        // 测试管理器配置
        let config = NetworkManagerConfiguration()
        networkmanager.configure(with: config)
        // XCTAssertTrue(networkmanager.isConfigured, "管理器应该配置成功")}
