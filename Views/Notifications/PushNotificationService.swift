...
class PushNotificationService: NSObject, ObservableObject {
    ...
    private func registerDeviceWithServer(token: String) async {
        do {
            ...
            await MainActor.run {
                ...
                if response.success {
        // print("设备注册成功: \(token)") // 调试语句已注释
                } else {
        // print("设备注册失败: \(response.message ?? "未知错误")") // 调试语句已注释
                }
            }
    ...
    func connectWebSocket() {
        guard let url = URL(string: ConfigurationManager.shared.pushNotificationWebSocketURL) else {
            return
        }
    ...
    private func handleWebSocketDataMessage(_ data: Data) {
        // 处理二进制数据（如果需要）
        // print("收到WebSocket二进制数据: \(data.count) bytes") // 调试语句已注释
    }
    ...
    func subscribeToStock(_ stockCode: String) async {
        ...
        // print("已订阅股票通知: \(stockCode)") // 调试语句已注释
    ...
    func unsubscribeFromStock(_ stockCode: String) async {
        do {
            ...
            try await networkManager.updateSubscription(request: request)
        // print("已取消订阅股票通知: \(stockCode)") // 调试语句已注释
